...
const InstagramSectionForm = () => {
  ...
  const handleSimpan = async () => {
    ...
    try {
      ...
      if (res.data?.success && res.data?.data) {
        const data = typeof res.data.data === 'string' ? JSON.parse(res.data.data) : res.data.data;
        setIgProfile(data);
        showAlert('success', 'Informasi akun Instagram Anda telah diperbarui.', 'Berhasil');
      }
    } catch (error) {
  ...
  const handleSimpanEmbeds = async () => {
    ...
    try {
      ...
      if (res.data?.success && res.data?.data) {
        ...
        showAlert('success', 'Link embed postingan Instagram Anda telah diperbarui.', 'Berhasil');
      }
    } catch (error) {
  ...
  return (
    <div className="cb-ig-section-form">
      ...
      {/* Notifikasi simpan (sukses & gagal) memakai satu style: ThemeAlert. */}
      <ThemeAlert
